...
name: append_wiki
description: >
  현재 작업/세션에서 새로 알게 된 지식(아키텍처, 설계 결정, 디버깅 근본원인,
  재사용 패턴, 환경·운영 노트, 범위 외 finding)을 프로젝트 wiki(.omc/wiki)에
  기록·병합한다. 작업·마일스톤 완료 직후, 또는 "append_wiki" / "위키 기록" /
  "wiki에 정리" / "알게 된 내용 기록" 요청 시 사용한다. 코드·깃·CLAUDE.md가 이미
  담는 사실은 기록하지 않고, 비자명한 지식만 응축해 남긴다.
---
