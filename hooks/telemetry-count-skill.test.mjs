...
test('활성 + banker 스킬이면 used-skills.json 에 이름 union 기록(개인화용·전송 안 함·중복 무증가)', () => {
  const tmp = mkTmp();
  enable(tmp);
  runSkillCount(tmp, { tool_name: 'Skill', tool_input: { skill: 'banker:compact-copy' } });
  runSkillCount(tmp, { tool_name: 'Skill', tool_input: { skill: 'banker:compact-copy' } }); // 같은 이름 두 번.
  runSkillCount(tmp, { tool_name: 'Skill', tool_input: { skill: 'banker:ultraqa' } });
  const set = JSON.parse(readFileSync(usedSkills(tmp), 'utf8'));
  assert.ok(Array.isArray(set));
  assert.deepEqual([...set].sort(), ['banker:compact-copy', 'banker:ultraqa'], 'union(중복 없이 두 이름)');
});

test('banker 필터 미매치면 used-skills 도 미기록', () => {
  const tmp = mkTmp();
  enable(tmp);
  runSkillCount(tmp, { tool_name: 'Skill', tool_input: { skill: 'other:foo' } });
